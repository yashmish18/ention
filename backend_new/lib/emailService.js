...
class EmailService {
  ...
  async sendConfirmationEmail(user) {
    if (!this.enabled) {
      console.log('Email service disabled. Skipping confirmation email for:', user.email);
  ...
  async sendWelcomeEmail(user) {
    if (!this.enabled) {
      console.log('Email service disabled. Skipping welcome email for:', user.email);
      return { messageId: 'email_disabled' };
    }
  ...
  async sendPasswordResetEmail(user, resetToken) {
    if (!this.enabled) {
      console.log('Email service disabled. Skipping password reset email for:', user.email);
      return { messageId: 'email_disabled' };
    }
  ...
  async testConnection() {
    if (!this.enabled) {
      console.log('Email service is disabled - no configuration found');
      return false;
    }
